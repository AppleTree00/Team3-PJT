...
## **[BACKLOG-001] 이력서 관리 — 새 이력서 만들기 버튼 미작동 (2026-06-25)**

> **상태: ❌ 미해결 / 후순위 (중요도 낮음)**

### 증상
- `resume.html` 상단 우측 `+ 새 이력서 만들기` 버튼 및 하단 점선 카드 버튼 클릭 시
  `select.html` (업로드/직접입력 선택 화면)으로 이동하지 않고 오동작 중
- href를 `select.html?new=true` → `select.html` 로 수정했으나 현상 유지

### 원인 추정
- 로그인 세션 체크 또는 `blockIfDemo()` 인터셉트에 의한 이동 차단 가능성
- `select.html` 자체의 인증 리다이렉트 문제 가능성

### 처리 방침
- **후순위 작업으로 보류** — 시연 핵심 경로에 영향 없음
- 해결 전까지 `select.html`을 직접 URL로 접근하는 것으로 우회 가능
...
## **[T025] AI 코칭 Phase 1 — 맥락 기반 프롬프트 강화 (2026-06-25)**

### 배경
기존 코칭은 `sample_type: general` 고정으로 범용 피드백만 반환. 회원의 목표 직무·업종·경력 연차 맥락이 없어 실질적 어드바이스 부족.

### 구현 (DB 변경 없음, 사이드 이펙트 최소화)

| 항목 | 변경 파일 | 내용 |
|---|---|---|
| 프롬프트 강화 | `coaching_routes.py` | `_build_prompt()`에 `target_job·target_industry·career_level` 3파라미터 추가 → 값 있으면 `=== 지원자 맥락 ===` 블록으로 system prompt에 주입 |
| API 엔드포인트 | `coaching_routes.py` | `/api/coaching` POST에서 3필드 수신. `sample_type` 미등록 시 422 에러 → general 폴백으로 변경(사이드 이펙트 방지) |
| 코칭 UI | `builder.html` | Skills 탭 하단에 "AI 코칭 요청" 카드 추가 — 목표 직무(텍스트), 목표 업종(텍스트), 경력 단계(드롭다운) + 결과 표시 패널 |
| JS 함수 | `builder.html` | `requestAICoaching()` 전면 개선 — 전체 입력 필드에서 이력서 텍스트 조합, 맥락 필드 수집, 결과를 인라인 패널에 표시, 제공자 배지(OpenAI/Gemini/Mock) 표시 |

### 검증
- API 직접 호출: `target_job=백엔드 개발자`, `target_industry=IT 스타트업`, `career_level=주니어(2~4년)` → OpenAI 실제 응답, 맥락 반영 어드바이스 정상 반환 ✅
- 기존 코칭 API 호환성 유지 (필드 없으면 빈 문자열로 처리, 기존 동작과 동일) ✅

### 향후 검토 (Phase 2)
- 관리자가 업종별 코칭 가이드 텍스트를 DB에 등록 → 프롬프트 주입 방식
- 현재 시연에서 Phase 1 효과 확인 후 결정

---

## **[T026] 자기소개 섹션 추가 — builder.html + coaching_routes.py + resume_routes.py (2026-06-25)**

### 배경
이력서 데이터에 직무수행경험·지원동기·향후포부 서술 필드 부재 → AI 코칭이 "왜 이 직무를 원하는가" 맥락 없이 피드백 생성

### 구현 (DB 마이그레이션 없음 — 기존 `extra_json` 컬럼 활용)

| 파일 | 변경 내용 |
|---|---|
| `builder.html` | 스테퍼에 4번째 탭 "자기소개" 추가. `<div id="Cover">` 섹션에 textarea 3개(직무수행경험·지원동기·향후포부). `STEP_ORDER` 4개로 확장. `calcSectionProgress()` Cover 케이스 추가. `saveResume()` — `extra: {work_narrative, motivation, aspiration}` 포함. `requestAICoaching()` — cover 3필드를 resume_text 조합 + fetch body에 별도 전송 |
| `coaching_routes.py` | `_build_prompt()` 시그니처에 3파라미터 추가. `cover_block`(=== 자기소개 ===) 시스템 프롬프트에 주입. 엔드포인트에서 3필드 수신 |
| `resume_routes.py` | `_export_resume_html()` — `extra_json` 파싱 후 자기소개 HTML 섹션 생성. `{cover_html}` 플레이스홀더 HTML 내보내기 템플릿에 추가 |

### 검증
- 서버 정상 기동, 에러 없음 ✅
- `extra_json` 저장/로드 기존 API 호환 유지 ✅

---

## **[T027] 제출관리 칸반 — 가상 지원 3건 시드 + "지원 추가" 모달 구현 (2026-06-25)**

### 배경
- 제출관리 페이지 "새로 추가" 버튼이 `handleUnavailableFeature()` 로 막혀 있어 지원 등록 불가
- 칸반 보드가 항상 빈 상태 → 시연 시 제출·코칭 흐름 시연 불가

### 구현

| 파일 | 변경 내용 |
|---|---|
| `app.py` | 데모 계정(`demo@a4u.com`) 최초 기동 시 가상 지원 3건 자동 삽입: 카카오엔터프라이즈(지원완료) / 라인플러스(면접) / 쿠팡(초안) |
| `dashboard.html` | "지원 추가" 모달 HTML 추가 (회사명·포지션·상태·지원일·메모). `btn-add-application` → `openAddAppModal()` 연결. `submitAddApp()` — `POST /api/applications` 호출 후 칸반 새로고침. `loadApplications()` IIFE → 명명 함수 전환(재호출 가능). 재호출 시 칸반 컬럼 초기화 후 재렌더(중복 카드 방지) |

### 검증
- DB 쿼리: `demo@a4u.com` 계정 지원 3건 정상 삽입 확인 ✅
  - `[submitted]` 카카오엔터프라이즈(주) / AI 서비스 엔지니어
  - `[interview]` 라인플러스(주) / 백엔드 개발자 (Server-Side)
  - `[draft]` 쿠팡(주) / Senior Software Engineer
- 서버 정상 기동, 에러 없음 ✅
