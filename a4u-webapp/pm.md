...
* [x] 3종 샘플 이력서 데이터가 정상적으로 입력되고, AI 피드백이 자연스럽게 연출되는지 최종 동선 체크. (QA 보고서 섹션 11 참조 — 전체 플로우 PASS, Mock AI 코칭 정상 동작 확인)
...
## 11. QA 보고서 — 일반 계정 전체 플로우 (2026-06-24)

### 11.1 테스트 범위
일반 계정 사용자 기준: 회원가입 → 로그인 → 이력서 작성 → AI 코칭 → 저장 → 제출지원 → 새 이력서 작성 → 기존 이력서 수정/저장
...
| AI 코칭 (`POST /api/coaching` — resume_text 키, mock 모드 정상) | PASS |
...
| 로그인 페이지 UI, 회원가입 탭 | PASS |

### 11.3 발견된 버그 및 조치 결과 ✅ 전건 수정 완료
| ID | 심각도 | 현상 | 원인 | 조치 |
|---|---|---|---|---|
| BUG-001 | HIGH | `GET /api/resumes` 에서 사용자 이력서에 샘플 3개가 혼재 → 사용자가 자신의 이력서를 구분하기 어려움 | `list_resumes()`가 로그인 사용자에게 항상 샘플도 포함해서 반환 | 기본은 사용자 이력서만 반환, `?include_samples=true` 파라미터로 샘플 선택 포함 가능하도록 수정 |
| BUG-002 | MEDIUM | `POST /api/applications` 시 `applied_date` 필드 완전히 무시 — 지원일이 저장 안됨 | `JobApplication` 모델에 `applied_date` 컬럼 없음 | 모델에 `applied_date VARCHAR(20)` 추가, DB ALTER TABLE 마이그레이션, create/update 라우트 반영 |
| BUG-003 | LOW | `PUT /api/resumes/{id}` — 샘플 이력서 편집 시도 시 `is_sample` 보호 없이 `user_id` 불일치 403만 반환 → 에러 메시지 불명확 | `update_resume()` 에 `is_sample` 체크 누락 | `is_sample=True`이면 "샘플 이력서는 편집할 수 없습니다" 명확한 403 반환하도록 수정 |

### 11.4 잔여 참고사항 (버그 아님)
- AI 코칭: `GEMINI_API_KEY` / `OPENAI_API_KEY` 미설정 시 Mock 모드로 동작 (의도된 Fallback)
- 제출처 `submitted_at`: status=`submitted` 로 변경 시에만 자동 기록 (설계 사양)
- 이력서 인쇄: 브라우저 `window.print()` 기반, builder.html에서 직접 호출
- Tailwind CDN 경고: CDN 방식 사용 시 항상 표시, 기능 무관
